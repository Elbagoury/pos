...
class stock_requirement_memo(models.Model):
	...
	partner_id = fields.Many2one('res.users','Mail User')
		...
		if self.date:
			...
			for product in self.env['product.product'].search([]):
				...
				if product.qty_available < reorder_rule.product_min_qty:
					srm_line_list.append((0,0, {'product_temp_id':product.product_tmpl_id.id,'product_id':product.id,'product_code_no':product.inma_code_no,'unit':product.uom_id.id,'current_stock':product.qty_available}))
	...
	def print_report(self):
		if self.project_id and self.stock_requirement_product_ids:
			...
			sheet1.write(row, 9, ' ', style_center_align1)
			row = 7
			for stock_product in self.stock_requirement_product_ids:
				...
				if stock_product.product_id:
					sheet1.write_merge(row, row, 5, 8, str(stock_product.product_id.name)+'-'+str(stock_product.product_id.attribute_value_ids.attribute_id.name)+':'+str(stock_product.product_id.attribute_value_ids.name), style_header_left2)
				else:
	...
	def confirm(self):
		if self:
			...
			server_url = 'http://184.95.34.90:8069/web?#&view_type=form&model=purchase.order&menu_id=%s&action=%s'%(menu_id,action_id)
...
class stock_requirement_product(models.Model):
	...
	product_id = fields.Many2one('product.product','Specification')
